...
#ifndef CATLASS_EPILOGUE_BLOCK_EPILOGUE_PER_TOKEN_SITU_HPP
#define CATLASS_EPILOGUE_BLOCK_EPILOGUE_PER_TOKEN_SITU_HPP
...
template <uint32_t UB_STAGES_, class CType_, class LayoutPerTokenScale_, class DType_, class TileElemWiseMuls_,
          class TileCopy_>
class BlockEpilogue<EpilogueAtlasA2PerTokenDequantSituQuant<UB_STAGES_>, CType_,
                    Gemm::GemmType<float, LayoutPerTokenScale_>, DType_, TileElemWiseMuls_, TileCopy_>
{
public:
    using DispatchPolicy = EpilogueAtlasA2PerTokenDequantSituQuant<UB_STAGES_>;
    ...
    struct Params {
        ...
        Params(__gm__ ElementPerTokenScale *ptrPerTokenScale_, LayoutPerTokenScale const &layoutPerTokenScale_,
               __gm__ ElementD *ptrD_, LayoutD const &layoutD_, float beta_ = 1.0f, float linearBeta_ = 1.0f,
               bool enableLinearBeta_ = false)
            : ptrPerTokenScale(ptrPerTokenScale_),
              layoutPerTokenScale(layoutPerTokenScale_),
              ptrD(ptrD_),
              layoutD(layoutD_),
              beta(beta_),
              linearBeta(linearBeta_),
              enableLinearBeta(enableLinearBeta_)
        {}
    };
    ...
    BlockEpilogue(Arch::Resource<ArchTag> const &resource, int32_t n, Params const &params = Params{}) : params(params)
    {
        ...
        uint32_t mathTmpBytes = ((blockN * sizeof(float) + 31) / 32) * 32;
        ...
        for (uint32_t i = 0; i < UB_STAGES; ++i) {
            ...
            ubOffset += ChunkTileLen * sizeof(float);
            ubMathTmpList[i] = resource.ubBuf.template GetBufferByByte<uint8_t>(ubOffset);
            ubOffset += mathTmpBytes;

            // Reuse the existing buffers for quantization tail; only the activation path changes.
                    ...
                    uint32_t epilogueCoreNum = 40, Callback &&callback = Callback{})
    {
        ...
        for (uint32_t loopIdx = loopStartIdx; loopIdx < loopStartIdx + tasksForIdx; ++loopIdx) {
            ...
            auto &ubGateAct = ubCFp32ChunkNList[ubListId];
            auto &ubTmpVec = ubCFp32ChunkNAbsList[ubListId];
            // Reuse the same full-size float buffer for gate scaling, sigmoid, and the later reduction result.
            auto &ubReduceMax = ubCFp32ChunkNMaxList[ubListId];
            auto &sharedMathTmpBuffer = ubMathTmpList[ubListId];
            ...
            // Cast C to FP32 in UB so the activation path stays in float32.
            ...
            // Apply the per-token dequant scale before the activation split.
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Muls(ubCFp32, ubCFp32, perTokenScale, blockN);
            AscendC::PipeBarrier<PIPE_V>();

            // Split: [0, ChunkTileLen) is gate, [ChunkTileLen, blockN) is up.
            auto gate = ubCFp32;
            auto up = ubCFp32[ChunkTileLen];

            // Step 1: up branch, optional linear beta transform.
            if (params.enableLinearBeta) {
                AscendC::Muls(ubReduceMax, up, 1.0f / params.linearBeta, ChunkTileLen);
                AscendC::PipeBarrier<PIPE_V>();
                // Use the official high-level tanh API; it needs its own temporary buffer.
                AscendC::Tanh(ubTmpVec, ubReduceMax, sharedMathTmpBuffer, ChunkTileLen);
                AscendC::PipeBarrier<PIPE_V>();
                AscendC::Muls(ubTmpVec, ubTmpVec, params.linearBeta, ChunkTileLen);
                AscendC::PipeBarrier<PIPE_V>();
            } else {
                // Keep the up half unchanged when linear_beta is disabled.
                AscendC::Muls(ubTmpVec, up, 1.0f, ChunkTileLen);
                AscendC::PipeBarrier<PIPE_V>();
            }

            // Step 2: gate branch = beta * tanh(gate / beta).
            AscendC::Muls(ubReduceMax, gate, 1.0f / params.beta, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Tanh(ubGateAct, ubReduceMax, sharedMathTmpBuffer, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Muls(ubGateAct, ubGateAct, params.beta, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();

            // Step 3: gate branch sigmoid(gate).
            AscendC::Sigmoid(ubReduceMax, gate, sharedMathTmpBuffer, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();

            // Step 4: situ_a = gate_tanh * sigmoid(gate).
            AscendC::Mul(ubGateAct, ubGateAct, ubReduceMax, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();

            // Step 5: out = situ_a * up_act.
            AscendC::Mul(ubGateAct, ubGateAct, ubTmpVec, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();

            // Quantization process follows the existing swiglu tail.
            AscendC::Abs(ubTmpVec, ubGateAct, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();

            AscendC::ReduceMax<float>(ubReduceMax, ubTmpVec, ubReduceMax, ChunkTileLen, false);
            AscendC::PipeBarrier<PIPE_V>();
            ...
            AscendC::WaitFlag<AscendC::HardEvent::S_V>(0);
            AscendC::Muls(ubTmpVec, ubGateAct, 127.f / GMubDequantScale, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();

            AscendC::Cast(ubQuantS32, ubTmpVec, AscendC::RoundMode::CAST_RINT, ChunkTileLen);
            AscendC::PipeBarrier<PIPE_V>();
...
private:
    ...
    AscendC::LocalTensor<uint8_t> ubMathTmpList[UB_STAGES];
...
#endif  // CATLASS_EPILOGUE_BLOCK_EPILOGUE_PER_TOKEN_SITU_HPP
