...
constexpr uint64_t FUSED_DEEP_MOE_PROFILE_CORE_TYPE_AIC = 0ULL;
...
constexpr inline uint64_t PackProfileLaunchCounts(uint32_t launchCountCapacity, uint32_t launchCountCaptured)
{
    return (static_cast<uint64_t>(launchCountCapacity) << 32) | static_cast<uint64_t>(launchCountCaptured);
}

constexpr inline uint64_t PackProfileLayout0(uint16_t stageCount, uint16_t groupCountCapacity, uint16_t aicPerGroup,
                                             uint16_t aivPerGroup)
{
...
constexpr inline uint64_t PackProfileLayout1(uint32_t logicalCoreCountCapacity, uint32_t recordsPerLaunch)
{
    return (static_cast<uint64_t>(logicalCoreCountCapacity) << 32) | static_cast<uint64_t>(recordsPerLaunch);
}

constexpr inline uint32_t UnpackProfileLaunchCapacity(uint64_t launchCountsPacked)
{
    return static_cast<uint32_t>(launchCountsPacked >> 32);
}

constexpr inline uint32_t UnpackProfileLaunchCaptured(uint64_t launchCountsPacked)
{
    return static_cast<uint32_t>(launchCountsPacked & 0xFFFFFFFFULL);
}

constexpr inline uint16_t UnpackProfileStageCount(uint64_t layoutPacked0)
{
    return static_cast<uint16_t>((layoutPacked0 >> 48) & 0xFFFFULL);
}

constexpr inline uint16_t UnpackProfileGroupCountCapacity(uint64_t layoutPacked0)
{
    return static_cast<uint16_t>((layoutPacked0 >> 32) & 0xFFFFULL);
}

constexpr inline uint16_t UnpackProfileAicPerGroup(uint64_t layoutPacked0)
{
    return static_cast<uint16_t>((layoutPacked0 >> 16) & 0xFFFFULL);
}

constexpr inline uint16_t UnpackProfileAivPerGroup(uint64_t layoutPacked0)
{
    return static_cast<uint16_t>(layoutPacked0 & 0xFFFFULL);
}

constexpr inline uint32_t UnpackProfileLogicalCoreCountCapacity(uint64_t layoutPacked1)
{
    return static_cast<uint32_t>(layoutPacked1 >> 32);
}

constexpr inline uint32_t UnpackProfileRecordsPerLaunch(uint64_t layoutPacked1)
{
    return static_cast<uint32_t>(layoutPacked1 & 0xFFFFFFFFULL);
}

constexpr inline uint32_t GetFusedDeepMoeProfileLogicalCoreLinear(uint64_t coreType, uint64_t coreIdx)
{
